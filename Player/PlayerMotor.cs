...
public class PlayerMotor : EntityBehaviour<IPlayerState>
{
    ...
    private bool _isDifusing = false;
    private float _difusedTime = 0f;
    private float _difusingTime = 5f;
    ...
    public State ExecuteCommand(bool forward, bool backward, bool left, bool right, bool jump, float yaw, float pitch, bool ability1, bool ability2, bool difuse)
    {
        if (!state.IsDead)
        {
            Vector3 movingDir = Vector3.zero;
            if (!_isPlanting && !_isDifusing)
            {
                if (forward ^ backward)
                {
            ...
            if (_AT)
            {
                if (entity.IsOwner)
                {
                    if (!_isDifusing)
                    {
                        if (difuse)
                        {
                            if (BombController.CheckDifuse(transform.position))
                            {
                                _isDifusing = true;
                                _difusedTime = BoltNetwork.ServerTime + _difusingTime;
                                BombController.SetDifuser(entity);
                                GetComponent<PlayerCallback>().RaiseStartDifuseEvent(_difusedTime);
                            }
                        }
                    }
                    else
                    {
                        if (!difuse || BombController._IS_DIFUSED)
                        {
                            _isDifusing = false;
                            BombController.SetDifuser(null);
                        }
                        else
                        {
                            if (_difusedTime < BoltNetwork.ServerTime)
                            {
                                if (state.Money + 600 > 8000)
                                    state.Money = 8000;
                                else
                                    state.Money += 600;

                                GameController.Current.Difused();
                                BombController._IS_DIFUSED = true;
                                _isDifusing = false;
                                BombController.SetDifuser(null);
                                _difusedTime = 0;
                            }
                ...
                else if (entity.HasControl)
                {
                    if (_isDifusing)
                    {
                        ...
                        {
                            GUI_Controller.Current.PlantingProgress(1 + ((BoltNetwork.ServerTime - _difusedTime) / _difusingTime));
                            if (_difusedTime < BoltNetwork.ServerTime)
                            {
                                _difusedTime = 0;
                                _isDifusing = false;
